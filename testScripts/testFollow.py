...
class OffbPosCtl:
    ...
    def __init__(self, this_uav,leader_uav, H):
        ...
        leader_sub = rospy.Subscriber('/mavros'+ leader_uav + '/local_position/pose', PoseStamped, callback=self.leader_cb)
        follower_sub = rospy.Subscriber('/mavros'+ leader_uav + '/local_position/pose', PoseStamped, callback=self.follower_cb)

        state_sub = rospy.Subscriber('/mavros'+ this_uav + '/state', State, callback=self.state_cb)
...
if __name__ == "__main__":
    OffbPosCtl(sys.argv[1], sys.argv[2], float(sys.argv[3]))
